...
class Customer(models.Model):
    ...
    def open_partner_history_saving(self):
        '''
        This function returns an action that display invoices/refunds made for the given partners.
        '''
        action = self.env.ref('saving_management-master.saving_prestamo_esperando_aprobacion_action')
    ...
    ahorros_ids = fields.One2many("saving.management.saving", "afiliado_id", "Prestamos de Cliente" )
    identidad = fields.Char("Identidad")
    rtn = fields.Char("RTN")
    aportaciones_ids = fields.One2many("saving.aportaciones", "cliente_id", "Prestamos de Cliente", domain=[('state', '=', 'done')])
